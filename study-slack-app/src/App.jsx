...
발화를 모두 훑고, 배우면 실질적으로 도움이 되는 문장 최대 5개를 선별해 피드백해줘.

[선정 원칙]
"상대방이 이해할 수는 있지만, 네이티브처럼 말하려면 이렇게 고쳐야 한다"는 문장만 골라.
의미 전달에 문제없고 개선해도 큰 차이가 없는 문장은 제외해.

[선정 기준 — 우선순위순]
1. 콩글리시 구조: 한국어를 직역한 패턴. 뜻은 통하지만 원어민은 이렇게 말하지 않음
2. 어휘 업그레이드: 맞는 단어를 쓰고 있지만 더 자연스럽고 세련된 표현이 있는 경우
3. 표현 확장: 말하려는 내용은 맞는데 더 풍부하게 표현할 수 있는 구조가 있는 경우
4. 반복 습관: 같은 어색한 패턴을 반복해서 쓰는 경우 (한 번 다루면 전체에 적용)

[무조건 제외]
- like / you know / kind of / I mean / sort of 같은 필러 워드 — 구어체에서 자연스러움
- 관사(a/an/the), 단복수, 시제 실수 — 뜻이 통하면 제외
- 이미 충분히 자연스러운 문장
- Yeah / Okay / Right 등 짧은 반응어
- 한국어 발화, 잘린 발화
- 전사 오류 의심 문장 (아래 중 하나라도 해당하면 제외):
  • 단어가 앞뒤 문맥과 갑자기 맞지 않거나 의미가 튀는 경우
  • 발음이 비슷한 다른 단어로 잘못 인식된 것으로 보이는 경우
  • 문장 중간에 뜬금없는 단어가 끼어든 것처럼 보이는 경우
  • 말하려는 의도가 조금이라도 불확실하면 제외 — 확실한 문장만 선택
...
*1. [이 교정의 핵심을 한 구절로]*
❌ 원문: \`원문 문장\`
✅ 추천: \`원어민이 실제 대화에서 쓸 법한 자연스러운 표현\`
💡 이유: 한국어로 짧게 — 왜 더 자연스러운지, 어떤 상황에 쓰는지

(각 항목 사이 빈 줄 필수, 최대 5개)
피드백할 문장이 5개 미만이어도 억지로 채우지 마.
...
[피드백 톤] 교정보다 업그레이드 관점. "틀렸다"가 아니라 "이렇게 말하면 훨씬 자연스럽다" 위주.
구어체 우선. 전사 오류는 문맥으로 추정해서 처리.
